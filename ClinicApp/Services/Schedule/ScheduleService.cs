...
namespace ClinicApp.Services.Schedule
{
    public class ScheduleService : IScheduleService
    {
        ...
        public ScheduleService(IScheduleEntryRepository repository)
        {
        ...
        public IEnumerable<ScheduleEntry> GetEntriesByWeek(int week, string doctorId)
        {
            ...
            return _repository.ScheduleEntries
        ...
        public async Task<ScheduleEntry> GetByIdAsync(int id) => await _repository.GetByIdAsync(id);

        public async Task AddAsync(ScheduleEntry scheduleEntry)
        {
            if (IsValid(scheduleEntry)) await _repository.AddAsync(scheduleEntry);
        ...
        {
            if (IsValid(scheduleEntry)) await _repository.UpdateAsync(id, scheduleEntry);
            else throw new ArgumentException("An entry within the specified time range already exists.");
        }
        ...
        private bool IsValid(ScheduleEntry scheduleEntry)
        {
            ...
            var collidingEntries = _repository.ScheduleEntries
                .Include(entry => entry.Doctor)
